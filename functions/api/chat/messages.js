import { verifySession, getCookie } from "../../_lib/auth.js";
import { chatKey } from "../../_lib/chat.js";
import { sendPushToUser } from "../../_lib/fcm.js";

async function isFriend(env, myId, otherId) {
  const raw = await env.FEED_KV.get(`friend:${myId}:${otherId}`);
  return !!raw;
}

export async function onRequestGet({ request, env }) {
  const token = getCookie(request, "session");
  const payload = await verifySession(token, env.SESSION_SECRET);
  if (!payload || !payload.uid) {
    return new Response(JSON.stringify({ error: "未登录" }), {
      status: 401,
      headers: { "content-type": "application/json" },
    });
  }

  const url = new URL(request.url);
  const withId = (url.searchParams.get("with") || "").trim();
  if (!withId) {
    return new Response(JSON.stringify({ error: "参数不对" }), {
      status: 400,
      headers: { "content-type": "application/json" },
    });
  }

  const myId = payload.uid;
  if (!(await isFriend(env, myId, withId))) {
    return new Response(JSON.stringify({ error: "还不是好友" }), {
      status: 403,
      headers: { "content-type": "application/json" },
    });
  }

  const raw = await env.FEED_KV.get(chatKey(myId, withId));
  const messages = raw ? JSON.parse(raw) : [];
  return new Response(JSON.stringify({ messages }), {
    headers: { "content-type": "application/json" },
  });
}

export async function onRequestPost({ request, env, waitUntil }) {
  const token = getCookie(request, "session");
  const payload = await verifySession(token, env.SESSION_SECRET);
  if (!payload || !payload.uid) {
    return new Response(JSON.stringify({ error: "未登录" }), {
      status: 401,
      headers: { "content-type": "application/json" },
    });
  }

  try {
    const { to, text, imageUrl, fileUrl, fileName } = await request.json();
    const withId = (to || "").trim();
    const body = (text || "").trim();
    if (!withId || (!body && !imageUrl && !fileUrl)) {
      return new Response(JSON.stringify({ error: "参数不对" }), {
        status: 400,
        headers: { "content-type": "application/json" },
      });
    }
    if (body.length > 2000) {
      return new Response(JSON.stringify({ error: "消息太长了" }), {
        status: 400,
        headers: { "content-type": "application/json" },
      });
    }
    if (imageUrl && imageUrl.length > 2_000_000) {
      return new Response(JSON.stringify({ error: "图片太大了" }), {
        status: 400,
        headers: { "content-type": "application/json" },
      });
    }
    if (fileUrl && fileUrl.length > 6_000_000) {
      return new Response(JSON.stringify({ error: "文件太大了，最大支持 4MB" }), {
        status: 400,
        headers: { "content-type": "application/json" },
      });
    }

    const myId = payload.uid;

    const meRaw = await env.FEED_KV.get(`user:${myId}`);
    const me = meRaw ? JSON.parse(meRaw) : null;
    if (me && me.bannedChat) {
      return new Response(JSON.stringify({ error: "你已被封禁私聊" }), {
        status: 403,
        headers: { "content-type": "application/json" },
      });
    }

    if (!(await isFriend(env, myId, withId))) {
      return new Response(JSON.stringify({ error: "还不是好友" }), {
        status: 403,
        headers: { "content-type": "application/json" },
      });
    }

    const key = chatKey(myId, withId);
    const raw = await env.FEED_KV.get(key);
    const messages = raw ? JSON.parse(raw) : [];
    const message = {
      id: crypto.randomUUID(),
      from: myId,
      text: body,
      imageUrl: imageUrl || null,
      fileUrl: fileUrl || null,
      fileName: fileName || null,
      createdAt: Date.now(),
    };
    messages.push(message);
    const trimmed = messages.length > 300 ? messages.slice(messages.length - 300) : messages;
    await env.FEED_KV.put(key, JSON.stringify(trimmed));

    // Fire-and-forget push notification to the recipient; never blocks the response.
    const senderName = (me && (me.username || me.name)) || "有人";
    const preview = body || (imageUrl ? "[图片]" : fileUrl ? `[文件] ${fileName || ""}`.trim() : "");
    const pushPromise = sendPushToUser(env, withId, {
      title: senderName,
      body: preview.length > 80 ? preview.slice(0, 80) + "…" : preview,
      data: { type: "chat", from: myId },
    });
    if (typeof waitUntil === "function") waitUntil(pushPromise);
    else await pushPromise;

    return new Response(JSON.stringify({ ok: true, message }), {
      headers: { "content-type": "application/json" },
    });
  } catch (e) {
    return new Response(JSON.stringify({ error: "服务器错误", detail: String(e) }), {
      status: 500,
      headers: { "content-type": "application/json" },
    });
  }
}
